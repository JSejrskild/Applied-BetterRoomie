...
```{r}
euclidv2 <- function(a) {
  n_cols <- 27  # number of columns to compute distances for
...
```{r}
percent2 <- function(a) {
  person_routine <- apply(a[, 1:7], 1, mean)
  cleanliness <- apply(a[, 8:14], 1, mean)
  communal_life <- apply(a[, 15:17], 1, mean)
  social_life <- apply(a[, 18:24], 1, mean)
  communication <- apply(a[, 25:27], 1, mean)
...
v2percent1 <- percent(v2result1[1:2,2:28])
